...
public class BackgroundSOSync {
    ...
    public BackgroundSOSync() {
    }

    public BackgroundSOSync(boolean isRefresh) {
        this.isRefresh = isRefresh;
    }

    public BackgroundSOSync init(Context context, IBGSyncListener listener) {
    ...
    private void fetchPendingList(){
        try {
            ...
            LogManager.writeLogInfo("Bacground sync so called fetching data from sharedpreference ");
            if (set!=null&&!set.isEmpty()) {
                LogManager.writeLogInfo("Bacground so sync called posting datavault data");
                Constants.writeLogsToInternalStorage(context,"Distributor: "+ MSFAApplication.DISTRIBUTOR_NAME+"\n" +
                        "log - Bacground sync called posting datavault data ");
                postVaultData(set);
            }else{
                Constants.isBGSync =false;
                LogManager.writeLogInfo("Bacground so sync called no datavault data");
                Constants.writeLogsToInternalStorage(context,"Distributor: "+ MSFAApplication.DISTRIBUTOR_NAME+"\n" +
                        "log - Bacground sync called no datavault data");
//                postOfflineData();
            }
        } catch (Throwable e) {
            Constants.isBGSync =false;
              ...
              for (String value :set) {
                  try {
                      ...
                      if (jsonObject.getString(Constants.entityType).equalsIgnoreCase(Constants.SecondarySOCreate)) {
                          ...
                          LogManager.writeLogInfo("Bacground so sync called posting order entry data");
                      }
                  } catch (Throwable e) {
                      isPostInProgress =true;
                      Constants.writeLogsToInternalStorage(context,"Distributor: "+ MSFAApplication.DISTRIBUTOR_NAME+"\n" +
